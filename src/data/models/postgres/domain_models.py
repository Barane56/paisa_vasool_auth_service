# domain_models.py — shared domain ORM models (dispute, invoice, email, memory)
# These tables are owned by the dispute service; auth service has them for
# FK integrity and read access only.
...
class InvoiceData(Base):
    ...
    payment_matches = relationship(
        "MatchingPaymentInvoice", back_populates="invoice", lazy="select"
    )
...
class PaymentDetail(Base):
    ...
    payment_matches = relationship(
        "MatchingPaymentInvoice", back_populates="payment", lazy="select"
    )
    disputes = relationship(
        "DisputeMaster", back_populates="payment_detail", lazy="select"
    )
...
class MatchingPaymentInvoice(Base):
    ...
    payment_detail_id = Column(
        Integer,
        ForeignKey("payment_detail.payment_detail_id", ondelete="RESTRICT"),
        nullable=False,
    )
    invoice_id = Column(
        Integer,
        ForeignKey("invoice_data.invoice_id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    payment = relationship(
        "PaymentDetail", back_populates="payment_matches", lazy="joined"
    )
    invoice = relationship(
        "InvoiceData", back_populates="payment_matches", lazy="joined"
    )
...
class EmailInbox(Base):
    ...
    dispute_id = Column(
        Integer,
        ForeignKey(
            "dispute_master.dispute_id",
            ondelete="SET NULL",
            use_alter=True,
            name="fk_email_inbox_dispute_id",
        ),
        nullable=True,
    )
    ...
    dispute = relationship(
        "DisputeMaster",
        back_populates="routed_emails",
        foreign_keys=[dispute_id],
        lazy="select",
    )
    episodes = relationship(
        "DisputeMemoryEpisode", back_populates="source_email", lazy="select"
    )
...
class EmailAttachment(Base):
    ...
    email_id = Column(
        Integer, ForeignKey("email_inbox.email_id", ondelete="CASCADE"), nullable=False
    )
...
class DisputeType(Base):
    ...
    disputes = relationship(
        "DisputeMaster", back_populates="dispute_type", lazy="select"
    )
...
class DisputeMaster(Base):
    ...
    email_id = Column(
        Integer, ForeignKey("email_inbox.email_id", ondelete="RESTRICT"), nullable=False
    )
    invoice_id = Column(
        Integer,
        ForeignKey("invoice_data.invoice_id", ondelete="SET NULL"),
        nullable=True,
    )
    payment_detail_id = Column(
        Integer,
        ForeignKey("payment_detail.payment_detail_id", ondelete="SET NULL"),
        nullable=True,
    )
    customer_id = Column(String(100), nullable=False)
    dispute_type_id = Column(
        Integer,
        ForeignKey("dispute_type.dispute_type_id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    episodes = relationship(
        ...
        lazy="select",
        cascade="all, delete-orphan",
        order_by="DisputeMemoryEpisode.created_at",
    )
...
class DisputeAIAnalysis(Base):
    ...
    auto_response_generated = Column(
        Boolean, default=False, server_default=text("FALSE"), nullable=False
    )
    created_at = Column(
        TIMESTAMP(timezone=True), server_default=func.now(), nullable=False
...
class AnalysisSupportingRef(Base):
    ...
    analysis_id = Column(
        Integer,
        ForeignKey("dispute_ai_analysis.analysis_id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    ai_analysis = relationship(
        "DisputeAIAnalysis", back_populates="supporting_refs", lazy="joined"
    )
...
class DisputeMemoryEpisode(Base):
    ...
    open_questions_asked = relationship(
        "DisputeOpenQuestion",
        back_populates="asked_episode",
        foreign_keys="DisputeOpenQuestion.asked_in_episode_id",
        lazy="select",
    )
    open_questions_answered = relationship(
        "DisputeOpenQuestion",
        back_populates="answered_episode",
        foreign_keys="DisputeOpenQuestion.answered_in_episode_id",
        lazy="select",
    )
    memory_summaries = relationship(
        "DisputeMemorySummary", back_populates="covered_up_to_episode", lazy="select"
    )
...
class DisputeMemorySummary(Base):
    ...
    covered_up_to_episode_id = Column(
        Integer,
        ForeignKey("dispute_memory_episode.episode_id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    dispute = relationship(
        "DisputeMaster", back_populates="memory_summary", lazy="joined"
    )
    covered_up_to_episode = relationship(
        "DisputeMemoryEpisode", back_populates="memory_summaries", lazy="select"
    )
...
class DisputeOpenQuestion(Base):
    ...
    dispute = relationship(
        "DisputeMaster", back_populates="open_questions", lazy="joined"
    )
    asked_episode = relationship(
        "DisputeMemoryEpisode",
        foreign_keys=[asked_in_episode_id],
        back_populates="open_questions_asked",
        lazy="select",
    )
    answered_episode = relationship(
        "DisputeMemoryEpisode",
        foreign_keys=[answered_in_episode_id],
        back_populates="open_questions_answered",
        lazy="select",
    )
...
class DisputeActivityLog(Base):
    ...
    dispute = relationship(
        "DisputeMaster", back_populates="activity_logs", lazy="joined"
    )
...
class DisputeStatusHistory(Base):
    ...
    dispute = relationship(
        "DisputeMaster", back_populates="status_history", lazy="joined"
    )
