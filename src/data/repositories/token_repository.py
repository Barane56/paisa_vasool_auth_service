...
class RefreshTokenRepository:
    ...
    async def get_by_token(self, token: str) -> RefreshToken | None:
        try:
            result = await self.db.execute(select(RefreshToken).where(RefreshToken.refresh_token == token))
    ...
    async def create(self, user_id: int, jti: str, refresh_token: str, settings: Settings) -> RefreshToken:
        try:
            expires_at = datetime.now(timezone.utc) + timedelta(days=settings.JWT_REFRESH_TOKEN_EXPIRE_DAYS)
            record = RefreshToken(user_id=user_id, jti=jti, refresh_token=refresh_token, is_revoked=False, expires_at=expires_at)
            self.db.add(record)
            await self.db.flush()
    ...
    async def revoke_all_for_user(self, user_id: int) -> None:
        try:
            await self.db.execute(
                update(RefreshToken).where(RefreshToken.user_id == user_id, RefreshToken.is_revoked == False).values(is_revoked=True)
            )
            await self.db.flush()
